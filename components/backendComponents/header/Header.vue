				...
				<v-btn icon class="ms-3" color="lighten-2">
					...
					<v-menu activator="parent" anchor="bottom end" origin="auto" max-width="300">
						<v-list class="pa-5" elevation="10" rounded="lg">
							...
							<v-list-item v-for="(item, i) in messages" class="my-2 pa-3" :key="i" rounded="lg" :value="item" @click="href" :title="item.title" :subtitle="item.desc">
								<template v-slot:prepend>
									<v-avatar size="50">
										<v-img :src="`/images/users/${item.image}`" :alt="item.image" width="50"> </v-img>
									</v-avatar>
								</template>
							</v-list-item>
							<v-btn flat color="primary" class="mt-4" variant="tonal" block>ดูข้อความทั้งหมด</v-btn>
						</v-list>
					</v-menu>
				</v-btn>
				...
				<v-btn icon class="ms-3" color="lighten-2">
					...
					<v-menu activator="parent" max-width="300">
						<v-list class="pa-6" elevation="10" rounded="lg">
							...
							<v-list-item class="pa-3 mt-2" v-for="(item, i) in notifications" :key="i" :value="item" @click="href" rounded="lg" :title="item.title" :subtitle="item.desc">
								<template v-slot:prepend>
					...
					<v-menu activator="parent">
						<v-list class="pa-6" elevation="10" rounded="lg">
							...
							<div class="d-flex align-center my-4">
								<img src="/images/users/sirirat.jpg" alt="Julia Roberts" class="rounded-circle" width="90" />
								<div class="ml-4">
							...
							<v-list-item class="pa-3 mb-2" v-for="(item, index) in userprofile" :key="index" :value="index" :title="item.title" :subtitle="item.desc">
								<template v-slot:prepend>
